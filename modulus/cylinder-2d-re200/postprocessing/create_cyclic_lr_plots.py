...
def _plot_lr(figdir):
    """plot lr
    """
    steps = numpy.arange(0, 1000000, 100)
    lr1 = cyclic_exp_range(steps, 0.99998, 1e-2, 1e-6, 5000)
    lr2 = cyclic_exp_range(steps, 0.9999915, 1e-2, 1e-6, 5000)

    fig = pyplot.figure(figsize=(5, 2.5))
    fig.suptitle("Cyclic learning rate history")
    ax = fig.gca()
    ax.semilogy(steps, lr2, lw=1.5, alpha=0.7, label=r"Cyclic for $Re=40$")
    ax.semilogy(steps, lr1, lw=1.5, alpha=0.7, label=r"Cyclic for $Re=200$")
