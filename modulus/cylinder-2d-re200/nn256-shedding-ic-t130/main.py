...
def get_initial_constraint(nodes, cfg):
    ...
    invars = {field: {key: None for key in ["x", "y", "t"]} for field in ["u", "v", "p"]}
    outvars = {field: {field: None} for field in ["u", "v", "p"]}
    constraints = {field: None for field in ["u", "v", "p"]}

    # read in gridlines of u
    with _h5File(pathlib.Path(cfg.orgdir).joinpath("data", "grid.h5")) as dset:
        for field in ["u", "v", "p"]:
            ...
            # time
            invars[field]["t"] = numpy.full_like(invars[field]["x"], cfg.custom.tstart)

    # read in data of u and time
    with _h5File(pathlib.Path(cfg.orgdir).joinpath("data", "0026000.h5")) as dset:
        for field in ["u", "v", "p"]:
            outvars[field][field] = dset[field][...].reshape(-1, 1)  # data

    # constraints
    for field in ["u", "v", "p"]:
        constraints[field] = _PointwiseConstraint.from_numpy(
            nodes=nodes,
            invar=invars[field],
...
def get_boundary_constraints(nodes, geo, cfg):
    ...
    inlet = _PointwiseBoundaryConstraint(
        ...
        param_ranges={t: (cfg.custom.tstart, cfg.custom.tend)}
    )
