...
def get_petibm_data(origdir, tstart, fields):
    """Get PetIBM data.
    """
    invars = {field: {key: None for key in ["x", "y", "t"]} for field in fields}
    outvars = {field: {field: None} for field in fields}

    # read in gridlines of u
    with _h5File(origdir.joinpath("data", "grid.h5")) as dset:
        for field in fields:
            invars[field]["x"], invars[field]["y"] = numpy.meshgrid(dset[field]["x"], dset[field]["y"], indexing="xy")
            invars[field]["x"] = invars[field]["x"].reshape(-1, 1)
            invars[field]["y"] = invars[field]["y"].reshape(-1, 1)

            # time
            invars[field]["t"] = numpy.full_like(invars[field]["x"], tstart)

    # read in data of u, v
    with _h5File(origdir.joinpath("data", "0026000.h5")) as dset:
        for field in fields:
            outvars[field][field] = dset[field][...].reshape(-1, 1)  # data
...
def get_initial_constraint(nodes, cfg):
    """Get an interior pointwise constraint for initial conditions.
    """

    tbg, _ = process_domain(cfg.custom.t)
    invars, outvars = get_petibm_data(pathlib.Path(cfg.orgdir), tbg, ["u", "v"])
    constraints = {field: None for field in ["u", "v"]}
...
def get_residual_monitors(nodes, geo, cfg):
    ...
    invars, outvars = get_petibm_data(pathlib.Path(cfg.orgdir), tbg, ["u", "v"])
    outvars = {key: val[key] for key, val in outvars.items()}
    area_u = (xed - xbg) * (yed - ybg) / invars["u"]["x"].size
    area_v = (xed - xbg) * (yed - ybg) / invars["v"]["x"].size

    ic_u = _PointwiseMonitor(
        invar=invars["u"],
        output_names=["u"],
        metrics={
            "ic_u": lambda var: _torchsum(area_u * _torchabs(var["u"].detach().cpu()-outvars["u"]))
        },
        nodes=nodes,
    )

    ic_v = _PointwiseMonitor(
        invar=invars["v"],
        output_names=["v"],
        metrics={
            "ic_v": lambda var: _torchsum(area_v * _torchabs(var["v"].detach().cpu()-outvars["v"]))
        },
        nodes=nodes,
    )
...
def get_solver_domains(nodes, geo, cfg):
    ...
    for name, ic in get_initial_constraint(nodes, cfg).items():  # pylint: disable=invalid-name
        domain.add_constraint(ic, name=f"ic-{name}")
