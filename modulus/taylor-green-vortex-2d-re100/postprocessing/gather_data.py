...
def get_casedata(workdir, casename, mtype, rank=0):
    ...
    cfg.elapsedtimes = log_parser(workdir.joinpath(casename))
...
def get_walltime_errs(cfg, graph, fields, mtype, h5file, h5kwargs, rank=0):  # pylint: disable=too-many-locals
    ...
    grp.create_dataset(f"steps", data=steps, **h5kwargs)
    grp.create_dataset(f"elapsedtimes", data=elapsedtimes, **h5kwargs)
...
def process_single_case(
    workdir, casename, mtype, snapshots=True, simtimeerr=True, walltimeerr=True, rank=0
    ...
    cfg, graph = get_casedata(workdir, casename, mtype, rank)
...
if __name__ == "__main__":
    ...
    topdir = projdir.joinpath("taylor-green-vortex-2d-re100")
    ...
    for nl, nn, nbs, mtype in itertools.product(layers, neurons, nbss, mtypes):
        inps.put((topdir, f"nl{nl}-nn{nn}-npts{nbs}", mtype, True, True, True))
