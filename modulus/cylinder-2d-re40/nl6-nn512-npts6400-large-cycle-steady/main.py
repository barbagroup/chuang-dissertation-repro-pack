...
def get_computational_graph(cfg: _ModulusConfig):
    ...
    u_outlet = ConvectiveBC("u", 1.0, "convective_u", 2, False)
    v_outlet = ConvectiveBC("v", 1.0, "convective_v", 2, False)
...
def get_boundary_constraints(nodes, geo, cfg):
    ...
    # reference pressure (duplicated coordinates because Modulus not able to handle one point)
    refx = _torchtensor([[xed], [xed]])
    refy = _torchtensor([[0.0], [0.0]])
