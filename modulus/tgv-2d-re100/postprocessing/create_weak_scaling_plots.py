...
def create_weak_scaling_plots(arch, scsimdir, scoutdir, bsimdir, boutdir, figdir):
    ...
    for ngpus in gpus:
        ...
        logdata.append(log_parser(scsimdir.joinpath(cname)))

        with h5open(scoutdir.joinpath(f"{cname}-raw.h5"), "r") as h5file:
            h5data.append({
        ...
        cname = f"nl{nl}-nn{nn}-npts{nbs*ngpus}"
        compdata.append(log_parser(bsimdir.joinpath(cname)))

        with h5open(boutdir.joinpath(f"{cname}-raw.h5"), "r") as h5file:
            comph5data.append({
                "steps": h5file["walltime/steps"][...],
                "elapsedtimes": h5file["walltime/elapsedtimes"][...],
                "l2norms": h5file["walltime/l2norms/u/40.0"][...]
    ...
    fig.savefig(figdir.joinpath("scaling-tests", f"nl{nl}-nn{nn}-npts{nbs}-weak-scaling.png"))

    # ===============================================================
    fig = pyplot.figure(figsize=(6.5, 5.3))
    fig.suptitle(r"Comparing $(N_l, N_n, N_{bs}) \times N_{gpu}$ and $(N_l, N_n, N_{bs} \times N_{gpu})$")
    gs = fig.add_gridspec(2, 2)

    locs = [(0, 0), (0, 1), (1, 0)]
    for iloc, ngpus, ilogdata, icompdata, ih5data, icomph5data in zip(
            locs, gpus[1:], logdata[1:], compdata[1:], h5data[1:], comph5data[1:]):

        styles = styles_def()

        ax = fig.add_subplot(gs[iloc])
        ax.set_title(rf"$({nl}, {nn}, {nbs}) \times {ngpus}$ v.s. $({nl}, {nn}, {nbs*ngpus})$")
        ax.set_xlabel("Iteration")
        ax.set_ylabel(r"Loss or $L_2$ error of $u$")
        tax = ax.twinx()
        tax.set_ylabel("Run time (hours)")
    ...
    lgds = [
        lax.legend([l1, l2], labels, title="Loss", loc="upper center", bbox_to_anchor=(0.5, 1.05)),
        lax.legend([l3, l4], labels, title=r"$L_2$ err. of $u$", loc="center", bbox_to_anchor=(0.5, 0.5)),
        lax.legend([l5, l5], labels, title="Run time", loc="lower center", bbox_to_anchor=(0.5, -0.05)),
    ]
    lax.add_artist(lgds[0])
    lax.add_artist(lgds[1])
    lax.add_artist(lgds[2])
    ...
    # save
    figdir.joinpath("scaling-tests").mkdir(parents=True, exist_ok=True)
    fig.savefig(figdir.joinpath("scaling-tests", f"nl{nl}-nn{nn}-npts{nbs}-multi-singl-gpus.png"))
...
if __name__ == "__main__":
    ...
    _scoutdir = _projdir.joinpath("outputs", "exp-sum-scaling")
    _scsimdir = _projdir.joinpath("exp-sum-scaling")
    _boutdir = _projdir.joinpath("outputs", "base-cases")
    _bsimdir = _projdir.joinpath("base-cases")

    _figdir = _projdir.joinpath("figures")
    _figdir.mkdir(parents=True, exist_ok=True)

    create_weak_scaling_plots((2, 32, 8192), _scsimdir, _scoutdir, _bsimdir, _boutdir, _figdir)
    create_weak_scaling_plots((3, 128, 8192), _scsimdir, _scoutdir, _bsimdir, _boutdir, _figdir)
