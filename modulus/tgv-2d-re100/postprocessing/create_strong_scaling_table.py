...
def create_strong_scaling_table(simdir, outdir, tabledir):
    """Create strong scaling table.
    """

    gpus = [1, 2, 4, 8]
    cases = [(2, 32, 65536), (3, 128, 65536)]
    indices = ["Time cost", "Speedup", "Efficiency", "Loss", r"$L_2$ err., $u$", r"$L_2$ err., $v$"]
    ...
    for (nl, nn, nbs), ngpus in itertools.product(cases, gpus):
        ...
        data.loc["Speedup", ((nl, nn, nbs), ngpus)] = \
            data.loc["Time cost", ((nl, nn, nbs), 1)].values[0] / \
        ...
        data.loc["Efficiency", ((nl, nn, nbs), ngpus)] = \
            ...
            data.loc["Time cost", ((nl, nn, nbs), ngpus)].values[0] / ngpus

    out = data.style
    out = out.format(formatter="{:5.2f}", subset=pandas.IndexSlice["Time cost", :])
    out = out.format(formatter="{:3.1f}x", subset=pandas.IndexSlice["Speedup", :])
    ...
    with open(tabledir.joinpath("scaling-tests", "strong-scaling.tex"), "w") as fobj:
        fobj.write(out)
...
if __name__ == "__main__":
    ...
    create_strong_scaling_table(_simdir, _outdir, _tabledir)
